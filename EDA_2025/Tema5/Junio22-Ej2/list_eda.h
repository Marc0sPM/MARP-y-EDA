...
public:
   ...
   void adelantaPosiciones(int const pos, int lon, int const k) {
       int n = this->nelems;
       if (this->empty() || lon == 0 || k == 0 || pos < 0 || pos >= this->nelems) return;

       if (pos + lon > n) lon = n - pos;
       if (lon <= 0) return;

       if (k > pos) return; // No se puede adelantar más allá del principio
       int newPos = pos - k;
       if (newPos == pos) return; // No hay que mover nada

       Nodo* firstList = this->fantasma->sig;
       Nodo* firstSeg = firstList;
       for (int i = 0; i < pos; ++i)
           firstSeg = firstSeg->sig;

       Nodo* lastSeg = firstSeg;
       for (int i = 1; i < lon; ++i)
           lastSeg = lastSeg->sig;

       Nodo* preSeg = firstSeg->ant;
       Nodo* postSeg = lastSeg->sig;

       preSeg->sig = postSeg;
       postSeg->ant = preSeg;

       Nodo* destino = firstList;
       for (int i = 0; i < newPos; ++i)
           destino = destino->sig;

       Nodo* preDestino = destino->ant;
       preDestino->sig = firstSeg;
       firstSeg->ant = preDestino;
       lastSeg->sig = destino;
       destino->ant = lastSeg;
   }
};
