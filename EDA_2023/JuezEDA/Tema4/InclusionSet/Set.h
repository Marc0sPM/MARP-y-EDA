...
public:
    ...
    //INCLUSIÓN
    bool operator<=(const Set<T>& set) const { //O(n), siendo n MIN[set1.size(), set2.size()]
        int itSet1 = 0, itSet2 = 0;
        while(itSet1 != this->size() && itSet2 != set.size()) {
            if(this->array[itSet1] == set.array[itSet2]) {
                ++itSet1;
                ++itSet2;
            }
            else if(this->array[itSet1] > set.array[itSet2]) ++itSet2;
            else if(this->array[itSet1] < set.array[itSet2]) return false;
        }
        if(itSet1 != this->size()) return false;
        else return true;
    }
