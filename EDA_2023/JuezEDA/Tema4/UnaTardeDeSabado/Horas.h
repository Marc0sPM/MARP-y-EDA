...
public:
    ...
    Horas operator+(const Horas& other) const {
        Horas tmp;
        int horas = 0, minutos = 0, segundos = 0;
        segundos = this->segundos + other.segundos;
        minutos = this->minutos + other.minutos;
        horas = this->horas + other.horas;
        if(segundos >= 60) {
            minutos += segundos/60;
            segundos = segundos % 60;
        }
        tmp.segundos = segundos;
        ...
        if(horas >= 24) throw std::range_error("ERROR");
        tmp.horas = horas;
